...
class BaseService(ExceptionHandlingServiceBase):
    ...
    async def get_entities_by_ids_test(
        self, model_class: Type[ModelType], ids: List[int]
    ) -> List[ModelType]:
        """
        Get entities by their IDs with validation.
    ...
    async def update_many_to_many_relationship_test(
        self,
        relationship_collection: List[Any],
        model_class: Type[ModelType],
        related_ids: List[int],
        ...
        # Add new items
        if ids_to_add:
            entities_to_add = await self.get_entities_by_ids_test(model_class, list(ids_to_add))
            for entity in entities_to_add:
                relationship_collection.append(entity)
